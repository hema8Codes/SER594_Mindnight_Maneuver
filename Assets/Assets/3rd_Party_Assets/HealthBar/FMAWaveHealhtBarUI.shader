...
Shader "AlchemistLab/UI/WaveHealthBar" {
	Properties {
	...
	SubShader {
		...
		Pass{
			...
			fixed4 frag(v2f input) : SV_Target
			{
				...
				float val1 = _Value + min(_Value, 1 - _Value) * _WaveAmplitude * sin(_Time * _WaveFrequency + input.texcoord.x * _WaveSize);
				val1 = smoothstep((1 - val1) - 0.01, (1 - val1) + 0.01, maskValue);
				float val2 = _Value - min(0.2 + _Value, 0.2 + 1 - _Value) * _WaveAmplitude * 1.7 * cos(2 - _Time * _WaveFrequency * 0.77f + input.texcoord.x * _WaveSize * 0.7);
